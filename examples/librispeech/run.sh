...
if [ $stage -le 0 ]; then
  ...
  utils/combine_data.sh data/${valid_set} data/dev_clean data/dev_other
fi
...
if [ ${stage} -le 6 ]; then
  ...
  for dataset in $test_set; do
    log_file=$dir/logs/dump_$dataset.log
    result_file=$dataset/posteriors.ark
    mkdir -p $dir/$dataset
    python3 test.py \
	    --test data/${dataset}_${type}${unit}.json \
